...
export interface ValidationIssue {
  ...
  field?: string;
  /**
   * Stable i18n key (relative to the `Flows.validation` namespace) used
   * by the UI to render a localized message. `message` is kept as an
   * English fallback for logs / tests / non-UI consumers.
   */
  code: string;
  /** Interpolation values for the localized message. */
  params?: Record<string, string | number>;
  /** English fallback message (kept for tests + server logs). */
  message: string;
}
